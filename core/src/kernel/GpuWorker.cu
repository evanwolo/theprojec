...
__global__ void updateBeliefsKernel(AgentDataView view, float stepSize, float simFloor) {
    ...
    float myB0 = view.B0[i];
    float myB1 = view.B1[i];
    float myB2 = view.B2[i];
    float myB3 = view.B3[i];
    float mySusc = view.susceptibility[i];
    float myFluency = view.fluency[i];
    std::uint8_t myLang = view.primaryLang[i];

    float myNormSq = myB0*myB0 + myB1*myB1 + myB2*myB2 + myB3*myB3;
    ...
    const float* __restrict__ B0 = view.B0;
    const float* __restrict__ B1 = view.B1;
    const float* __restrict__ B2 = view.B2;
    const float* __restrict__ B3 = view.B3;
    const float* __restrict__ sus = view.susceptibility;
    const float* __restrict__ flu = view.fluency;
    ...
    for (int j = 0; j < count; ++j) {
        ...
        // Load Neighbor State
        float tB0 = __ldg(B0 + nbrId);
        float tB1 = __ldg(B1 + nbrId);
        float tB2 = __ldg(B2 + nbrId);
        float tB3 = __ldg(B3 + nbrId);
        if (!view.alive[nbrId]) continue;
        float nbrFluency = __ldg(flu + nbrId);
        std::uint8_t nbrLang = view.primaryLang[nbrId];
        
        // --- Physics Logic (Identical to CPU version) ---
        float tNormSq = tB0*tB0 + tB1*tB1 + tB2*tB2 + tB3*tB3;
        float dot = myB0*tB0 + myB1*tB1 + myB2*tB2 + myB3*tB3;
        
        float sim = 1.0f;
        float normProd = myNormSq * tNormSq;
        if (normProd > 1e-9f) {
            sim = dot * rsqrtf(normProd);
        }
        
        // Similarity Gate
        float gate = (sim - simFloor) / (1.0f - simFloor);
        if (gate <= 0.0) continue; // Optimization: Skip calculation

        // Language Factor
        float langQ = (myLang == nbrLang) ? 0.5f * (myFluency + nbrFluency) : 0.1f;
        if (langQ <= 0.0) continue;
        
        float weight = stepSize * gate * langQ * mySusc;

        // Apply Force using Fast Tanh Approximation
        auto fastTanh = [] __device__ (float x) { 
            float x2 = x*x; 
    ...
    auto clamp = [] __device__ (float v) { return (v < -1.0f) ? -1.0f : (v > 1.0f) ? 1.0f : v; };
...
struct GpuContext {
    ...
    float *dB0 = nullptr, *dB1 = nullptr, *dB2 = nullptr, *dB3 = nullptr;
    float *dSus = nullptr, *dFlu = nullptr;
    ...
    void resize(size_t nAgents, size_t nEdges) {
        ...
        CUDA_CHECK(cudaMalloc(&dB0, nAgents * sizeof(float)));
        CUDA_CHECK(cudaMalloc(&dB1, nAgents * sizeof(float)));
        CUDA_CHECK(cudaMalloc(&dB2, nAgents * sizeof(float)));
        CUDA_CHECK(cudaMalloc(&dB3, nAgents * sizeof(float)));
        CUDA_CHECK(cudaMalloc(&dSus, nAgents * sizeof(float)));
        CUDA_CHECK(cudaMalloc(&dFlu, nAgents * sizeof(float)));
...
void launchGpuBeliefUpdate(AgentStorage& hostStorage, const KernelConfig& cfg) {
    ...
    // 2. Copy Host -> Device (PCIe Transfer) only when data changed
    if (hostStorage.needsBeliefUpload()) {
        CUDA_CHECK(cudaMemcpy(g_ctx.dB0, hView.B0, nAgents * sizeof(float), cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(g_ctx.dB1, hView.B1, nAgents * sizeof(float), cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(g_ctx.dB2, hView.B2, nAgents * sizeof(float), cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(g_ctx.dB3, hView.B3, nAgents * sizeof(float), cudaMemcpyHostToDevice));
        hostStorage.markBeliefsClean();
    }

    if (hostStorage.needsPropertyUpload()) {
        CUDA_CHECK(cudaMemcpy(g_ctx.dSus, hView.susceptibility, nAgents * sizeof(float), cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(g_ctx.dFlu, hView.fluency, nAgents * sizeof(float), cudaMemcpyHostToDevice));
        ...
        hostStorage.markPropertiesClean();
    }

    if (hostStorage.needsGraphUpload()) {
    ...
    updateBeliefsKernel<<<blocks, threads>>>(dView, static_cast<float>(cfg.stepSize), static_cast<float>(cfg.simFloor));
    
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());

    // 5. Copy Device -> Host (PCIe Transfer)
    CUDA_CHECK(cudaMemcpy(hView.B0, g_ctx.dB0, nAgents * sizeof(float), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(hView.B1, g_ctx.dB1, nAgents * sizeof(float), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(hView.B2, g_ctx.dB2, nAgents * sizeof(float), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(hView.B3, g_ctx.dB3, nAgents * sizeof(float), cudaMemcpyDeviceToHost));
}
